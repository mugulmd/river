...
def _weighted_var(X: typing.Sequence[float], W: typing.Sequence[float], ddof: int) -> float:
    X_arr = np.asarray(X)
    average = np.average(X_arr, weights=W)
    return float(np.average((X_arr - average) ** 2, weights=W) * len(X) / (len(X) - ddof))

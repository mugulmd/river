...
__all__ = ["Bivariate", "Link", "NotEnoughSamples", "RollingUnivariate", "Statistic", "Univariate"]
...
class NotEnoughSamples(Exception):
    """Raised by `get` when a statistic hasn't seen enough values to produce one."""


class Statistic(abc.ABC, base.Base, typing.Generic[R]):
    """A statistic."""
    ...
    def __repr__(self) -> str:
        try:
            value = self.get()
        except (NotImplementedError, NotEnoughSamples):
            return f"{self.__class__.__name__}: None"
        try:
            fmt_value = f"{value:{self._fmt}}".rstrip("0")
        except (TypeError, ValueError):
            # The value isn't necessarily a float, e.g. the mode of categorical values.
            fmt_value = repr(value)
...
class Univariate(Statistic[R], typing.Generic[T, R]):
...
class Link(Univariate[T, S], typing.Generic[T, R, S]):
    ...
    values, and therefore cannot produce a value. The `stats.Mean` instance is therefore not
    updated.
    ...
    def __init__(self, left: Univariate[T, R], right: Univariate[R, S]):
        self.left = left
        self.right = right

...
class Shift(stats.base.Univariate):
    ...
    *without* including today's sales. You can do this by prepending an instance of
    ...
    def __init__(self, amount: int = 1, fill_value: float | None = None):
    ...
    def get(self) -> float:
        try:
            ...
            raise stats.base.NotEnoughSamples(
                f"{self.name} needs {self.amount + 1} values, has seen {len(self.buffer)}"
            ) from None
